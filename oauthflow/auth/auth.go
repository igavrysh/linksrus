...
func (ar *Result) Client(ctx context.Context) (*http.Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to exchange authentication code with OAuth token: %w", err)
	}
...
func NewOAuthFlow(cfg oauth2.Config, callbackListenAddr, redirectHost string) (*Flow, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cannot create listener for handling OAuth redirects: %w", err)
	}
	if redirectHost == "" {
...
func (f *Flow) Close() error {
	...
	for _, resCh := range f.pendingRequests {
		resCh <- Result{
			authErr: errors.New("authentication handler has been closed"),
		}
	}
...
func genNonce(length int) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("unable to generate random nonce for oauth flow: %w", err)
	}
