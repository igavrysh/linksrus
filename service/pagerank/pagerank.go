...
import (
	...
	"github.com/igavrysh/linksrus/bspgraph"
	"github.com/igavrysh/linksrus/linkgraph/graph"
	pr "github.com/igavrysh/linksrus/pagerank"
...
func (svc *Service) loadEdges(fromID, toID uuid.UUID, filter time.Time) error {
	...
	for edgeIt.Next() {
		...
		if err = svc.calculator.AddEdge(edge.Src.String(), edge.Dst.String());
			err != nil && !errors.Is(err, bspgraph.ErrUnknownEdgeSource) {
			_ = edgeIt.Close()
			return err
		}
	}
